...
const SelectItem = <T extends string | number>({
  ...
  return (
    <div className="custom-scrollbar border-slate-500 bg-slate-700 absolute left-0 right-0 z-50 max-h-36 translate-y-[-4px] overflow-y-auto rounded-sm border-[1px] border-solid">
      <ul>
        {items.map((item) => (
          <li
            key={item}
